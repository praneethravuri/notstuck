              ...
              {relevantChunks.length === 0 ? (
                ...
                <div className="space-y-2">
                  {relevantChunks.map((chunk, index) => (
                    <div
                      key={index}
                      className="border border-gray-800 bg-stone-900/50 rounded-lg overflow-hidden"
                    >
                      <Accordion type="single" collapsible>
                        <AccordionItem value={`item-${index}`} className="border-none">
                          <AccordionTrigger className="px-4 py-3 hover:no-underline hover:bg-stone-800/50">
                            <div className="flex items-center gap-2">
                              <FileText className="h-4 w-4 text-green-500" />
                              <span className="text-sm text-gray-200">
                                {sources[index] || `Source ${index + 1}`}
                              </span>
                            </div>
                          </AccordionTrigger>
                          <AccordionContent>
                            <div className="px-4 py-3 text-sm text-gray-400 bg-stone-900/30">
                              {chunk}
                            </div>
                          </AccordionContent>
                        </AccordionItem>
